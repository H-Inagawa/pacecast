...
- ORM: SQLAlchemy 2.0
- 天気予報: Open-Meteo（API キー不要）。地点は設定のアメダス観測所（未設定時は東京 44132）。テスト用の既存設定・サンプル CSV は練馬（44071）のまま
- 過去の風・日射: Open-Meteo 再解析。直近数日の気温・湿度・風はアメダス map JSON も使う
...
- 地点は設定のアメダス観測所。未設定時は東京（44132）。走行追加と予報では地点を選べる（既定は設定値）。CSV・既存テストデータは練馬（44071）
