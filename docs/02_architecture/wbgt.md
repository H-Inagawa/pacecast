...
WBGT は走行テーブルではなく `weather_observations` に持つ。走は開始時刻の気象 FK を見る。別の1時間値にまたがる走は終了時刻の FK も持ち、表示・予測では気温・湿度・風・日射の平均から推定 WBGT を付け直す。
...
未設定時の既定地点は東京（観測所 ID `44132`）。設定・走行追加・予報の地点は都道府県セレクトで絞り、一覧は観測所番号順。都道府県は気象庁の観測所番号（先頭2桁）から決める。設定には「GPSで探す」があり、現在地から最寄りの観測所を入れる。記録追加の初期地点は、設定どおりのアメダスか GPS 最寄りかをアカウント設定で選ぶ。GPS 不許可・測位失敗時は設定地点に戻す。位置情報は HTTPS か `http://127.0.0.1` で使える（通常の HTTP はブラウザが止める）。既存のテスト用設定は練馬（44071）のまま。手動 CSV は使わない。
