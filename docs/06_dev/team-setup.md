...
ブラウザは http://127.0.0.1:3000 です（`localhost` ではなく `127.0.0.1`）。起動できたかは次で確認します。`000` やタイムアウトなら、ポートは空いていてもサーバが止まっています。`README.md` の停止手順で落としてから `npm run dev` し直します。設定の「GPSで探す」は、この `127.0.0.1` か公開サイト（HTTPS）で使えます。`192.168.*` などの HTTP ではブラウザが位置情報を止め、ボタンは無効になります。
