...
新規登録の確認メールは、Next.js が `smtp.gmail.com`（ポート 587、STARTTLS）へ送ります。送信用は **PaceCast 専用の Google アカウント** にします。私用 Gmail は使わないでください。アプリパスワードが `.env` や Vercel に残るため、私用ログインと混ぜると止めたり回したりしにくいです。
