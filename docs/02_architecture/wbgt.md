...
| 要素 | 優先 |
| --- | --- |
| 気温・湿度 | 既存の気象庁 CSV → 直近のアメダス map JSON → Open-Meteo |
...
未設定時の既定地点は東京（観測所 ID `44132`）。設定・走行追加・予報のプルダウンは観測所番号順。サンプル CSV と既存のテスト用設定は練馬（44071）のまま。
...
走行一覧の気象列、気象データ画面、予測の根拠表に WBGT を出す。一覧の気象列ヘッダには説明アイコンを付ける。
