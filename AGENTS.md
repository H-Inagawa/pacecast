...
- 走行記録は月別。新しい月が上。見出し右に全体の件数と距離、各月も月名とサマリを同じ行に出す。WBGT 色分けのときは見出しと月の間に高さ 30px の凡例（凡例: / 暑すぎる / 暑い / 快適 / 寒い / 寒すぎる / 未適用）。表は横スクロールし、画面幅は広げない。各行にペースと、気象は気温・湿度・WBGTを別列で出す。未関連は `--.-℃` / `--%` / `--.-`。WBGT列ヘッダに説明アイコンを付ける。削除は「記録を削除してよろしいですか？」のモーダルで確認する
